...
<body>
    ...
    <div class="card mb-3 mx-5">
        <div class="row no-gutters">
            <div class="col-md-4">
                <img src="{{ asset($post->image) }}" alt="..." width="380" height="250" class="m-3">
            </div>
            <div class="col-md-8 my-auto">
                <div class="card-body">
                    <h2 class="card-title">{{ $post->title }}</h2>
                    <span class="card-text">Disusun Oleh: <span class="text-danger">{{ $post->users->name }}</span></span><br>
                    <span class="card-text">Kategori: <span class="text-danger">@foreach( $categories as $category)
                            @if($post->category_id == $category->id)
                            {{ $category->name }}
                            @endif
                            @endforeach</span></span><br>
                    <span class="card-text">Tag: <span class="text-light">@foreach($post->tags as $tag)
                            <span class="badge bg-dark my-1">{{ $tag->name }}</span>
                            @endforeach<br>
                            <p class="card-text"><small class="text-muted">Terakhir Diperbaharui {{ $post->tanggal }}</small></p>
                </div>
            </div>
        </div>
    </div>
